...
def find_blank_position(state): # Satrting point. where is the blank tile
...
def is_valid_move(x, y): # good move??
    return 0 <= x < SIZE and 0 <= y < SIZE

def perform_move(state, direction): # move the blank tile
...
def calculate_heuristic(state, goal): # incorrect tiles, how many tiles are in the wrong place.
    incorrect = sum(1 for x in range(SIZE) for y in range(SIZE)
                    if state[x][y] != goal[x][y] and state[x][y] != '*')
    return incorrect

def solve_puzzle(state, goal, steps, path=[], depth=0): # recursive function to check if its solved and not to pass the depth limit 
    if state == goal:
...
st.title('8 Puzzle Solver with Heuristic Backtracking') # streamlit stuff
...
if st.button('Solve Puzzle'): # button to solve the puzzle
    ...
    if solve_puzzle(initial_state_matrix, goal_state_matrix, steps): # if the puzzle is solved
